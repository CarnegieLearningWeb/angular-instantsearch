...
describe('createSSRSearchClient (v4)', () => {
  it('passes user agents', () => {
    ...
    expect(ssrSearchClient.transporter.userAgent.value).toBe(
      `Algolia for JavaScript (${algoliasearch.version}); Node.js (${process.versions.node}); angular (${AngularVersion.full}); angular-instantsearch (${VERSION}); angular-instantsearch-server (${VERSION})`
    );
  });
